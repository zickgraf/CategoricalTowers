...
InstallMethodWithCache( PreSheavesOfFpEnrichedCategory,
  ...
  function ( B, D )
    ...
    if ( IsFpCategory( B ) and HasIsFinitelyPresentedCategory( B ) and IsFinitelyPresentedCategory( B ) ) or
       ...
       IsAlgebroidFromDataTables( B ) then
        ...
        AddIsWellDefinedForMorphisms( PSh,
          function ( PSh, eta )
            local D, defining_triple, nr_objs, nr_mors, mors, F, G;
        ...
        if IsFpCategory( B ) then
          ...
          IsCategoryFromDataTables( B ) then
            ...
            AddIsWellDefinedForObjects( PSh,
              function ( PSh, F )
                local V, defining_triple, nr_objs, nr_mors, mors, relations, on_mors, is_equal;
                ...
                if not ForAll( [ 1 .. nr_objs ], o -> IsWellDefinedForObjects( V, ValuesOfPreSheaf( F )[1][o] ) ) then
                    return false;
                elif not ForAll( [ 1 .. nr_mors ], m -> IsWellDefinedForMorphisms( V, ValuesOfPreSheaf( F )[2][m] ) ) then
                    return false;
                elif not ForAll( [ 1 .. nr_mors ], m -> IsEqualForObjects( V, ValuesOfPreSheaf( F )[1][1 + mors[m][2]], Source( ValuesOfPreSheaf( F )[2][m] ) ) ) then
                    return false;
                elif not ForAll( [ 1 .. nr_mors ], m -> IsEqualForObjects( V, ValuesOfPreSheaf( F )[1][1 + mors[m][1]], Target( ValuesOfPreSheaf( F )[2][m] ) ) ) then
                    return false;
                fi;
        ...
        AddIsEqualForObjects( PSh,
          function ( PSh, F, G )
            local D, defining_triple, nr_objs, nr_mors, mors;
        ...
        AddIsEqualForMorphisms( PSh,
          function ( PSh, eta, epsilon )
            local D, defining_triple, nr_objs;
    ...
    if MissingOperationsForConstructivenessOfCategory( D, "IsElementaryTopos" ) = [ ] and
       ...
       IsIdenticalObj( D, RangeCategoryOfHomomorphismStructure( PSh ) ) then
